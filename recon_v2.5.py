...
--added support for calculating upper_bound (get_upper_bound())
--corrected table of contents adding 9. Plotting resample against observed and 10. Upper bound
--changed recon version to 2.4 throughout

v2.4:
--Fixed bug in reading in input files
--Now accepts species count files in which the same species is listed multiple times
--Removed bin_size.
"""
...
8. Resampling a fit
9. Plotting resample against observed
10. Upper bound
...
python recon_v2.5.py -D -Q 0 1 inf -b error_bar_parameters.txt -o
...
python recon_v2.5.py -p -q 0 -o test_power_table.txt error_bar_parameters.txt
...
python recon_v2.5.py -r -o test_sample_1_resample.txt
...
python recon_v2.5.py -x --x_max 30 -o test_sample_1_plotfile.txt -b error_bar_parameters.txt test_sample_1_fitfile.txt
...
10. Upper bound (-Umax, --upper_bound)
--------------------------------------

Example:

python recon_v2.5.py -Umax -ff FIT_FILE -df D_NUMBER_FILE -N population_size -ss sample_size

Any reconstruction of missing species using a small sample from a 
large population suffers from a fundamental limitation. Species that 
are too rare to have an appreciable chance of appearing in the sample 
cannot be estimated based upon the sample. As shown by Mao and Lindsay, 
this results in upper confidence intervals for the missing species that 
are formally infinite.

As discussed, Recon addresses this problem by only estimating those 
species that are large enough to have an appreciable chance of 
influencing the sample distribution in a meaningful way. (Note that 
while mixing distributions are often approximated as continuous, in 
reality they are discrete, so smallest fitted population will often be 
practically meaningful.) In many cases this estimate will be of 
interest.

But this still leaves the estimate for all species unbounded. The 
number of individuals in a population is of course an upper bound for 
the number of species. In many cases of interest, such as analysis of 
immune repertoires, it is relatively easy to obtain reasonable 
estimates of the total number of individuals. For example, an estimate 
of total cells can be obtained by scaling a cell count against total 
tissue or blood volume, e.g., 1010 B cells in the body.

Below we show how the Recon fit can be combined with an estimate of 
the number of all individuals in a population to get a sharper upper 
bound on the number of all species.

Recon produces an overall clone-size distribution. The smallest clone 
size in this distribution is described by two parameters: the fraction 
of all clones that are of this size, wmin, and a mean number of cells 
that it contributes to the sample, mmin. Clone sizes smaller than this 
contribute a mean of zero cells to the sample; however, it is possible 
that there are smaller clones in the parent population, clones so small 
that they both do not contribute to the sample and are invisible to our 
algorithm. Recon’s estimate of the number of missing clones would not 
count such clones because it is not necessary to assume that they exist 
in order to obtain the observed sample clone-size distribution. 
However, if they were to exist, they would result in an undercount of 
the species richness in the parent. The goal in this section is to 
bound this potential undercount. One can then test its plausibility, as
described in the main text.

The maximum undercount Umax, and therefore the desired upper bound, is 
obtained for the case that all the cells in clones smaller than mmin 
are actually singlets. How many would that be? The answer is given by

U_max = R * w_min * m_min * N / S

where R is (Recon’s upper bound of) the overall species richness 
estimate, N is the total number of cells in the overall repertoire, and 
S is the sample size. Note the ratio S/N is the fraction of cells in 
the overall population that are sampled; scaling m_min by S/N (yielding 
m_min*N/S) thus gives the smallest clone size in the overall repertoire 
that Recon can distinguish from singlets.

See Supplementary Information for Kaplinsky and Arnaout Nat. Comm. 
2016 for more details and examples (https://static-content.springer.com/esm/art%3A10.1038%2Fncomms11881/MediaObjects/41467_2016_BFncomms11881_MOESM1711_ESM.pdf)
